...
2. sort — sorts the IPs so identical values are adjacent.
3. uniq -c — collapses duplicates and counts occurrences per unique IP.
4. sort -nr — numerically sorts the counts in descending order (most frequent first).
5. head -n 10 — shows the top 10 most active IPs hitting the server.
